...
const enemyTable = [
    ...
    { name: 'あくまのきし',   hp: 70,  maxHp: 70,  attack: 94,  defense: 82,  agility: 82,  evasion: 1,  exp: 54,  gold: 165, sprite: { x: 56,  y: 401, w: 48, h: 50 }, pattern: ['attack', 'attack', 'rarihoo'] },
...
// =====================================================================
// 本家FC版のエンカウントテーブル(ID 0〜D)の構成をそのまま移植したもの。
// 1段ごとに新顔が1種だけ増え、古株が数段かけて抜けていく緩やかな階段になる。
// ダンジョン専用だった敵(メーダ/メトロゴースト/ドロル/ドラキーマ/ヘルゴースト/
// メーダロード/ドロルメイジ)は、本作にダンジョンが無いので強さの近い段に混ぜている。
const zoneEnemyNames = [
    ...
    ['ゴースト', 'まほうつかい', 'メイジドラキー', 'おおさそり', 'メーダ'],
    ['メイジドラキー', 'おおさそり', 'メーダ', 'メトロゴースト', 'がいこつ'],
    ['おおさそり', 'がいこつ', 'ドロル', 'ドラキーマ', 'まどうし'],
    ['がいこつ', 'まどうし', 'てつさそり', 'リカント', 'ヘルゴースト'],
    ['てつさそり', 'リカント', 'しりょう', 'リカントマムル', 'ゴールドマン'],
    ['しりょう', 'リカントマムル', 'メーダロード', 'ドロルメイジ', 'キメラ'],
    ['キメラ', 'しのさそり', 'しりょうのきし', 'よろいのきし', 'メタルスライム'],
    ['よろいのきし', 'メイジキメラ', 'キラーリカント', 'スターキメラ', 'だいまどう'],
    ['あくまのきし', 'しにがみのきし', 'ストーンマン', 'ドラゴン', 'キースドラゴン', 'ダースドラゴン']
];
const zoneEnemySets = zoneEnemyNames.map(names => names.map(n => enemyByName[n]));
const ZONE_CELL = 8; // 区画の一辺(タイル)
// 城からの歩行距離(BFS)で14段に区切ったもの。13は虹の橋の先＝りゅうおう領域と海
const encounterZoneGrid = [
    [13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13],
    [13,4,4,3,3,2,3,3,3,7,7,7,6,6,6,7,13],
    [13,4,3,3,2,2,2,3,3,3,6,6,6,6,6,6,13],
    [13,3,3,2,2,2,2,2,3,3,3,6,5,5,6,6,13],
    [13,3,3,3,2,1,1,2,2,3,4,4,4,5,5,6,13],
    [13,4,4,4,1,1,0,1,2,3,3,4,4,5,5,5,13],
    [13,4,4,1,1,0,0,0,13,3,3,6,6,5,5,6,13],
    [13,4,4,1,1,1,13,13,13,7,6,6,6,5,5,6,13],
    [13,5,5,6,6,1,13,13,13,13,7,6,7,6,6,6,13],
    [13,5,6,6,6,7,7,13,13,13,7,7,7,7,7,6,13],
    [13,8,7,7,7,7,7,13,13,12,12,7,7,7,7,7,13],
    [13,8,8,7,7,7,8,10,10,11,12,12,8,8,7,7,13],
    [13,8,8,8,8,10,10,10,12,12,12,12,13,8,8,7,13],
    [13,9,8,8,9,9,11,11,12,12,12,12,12,8,8,8,13],
    [13,9,9,9,9,10,10,11,11,12,12,12,12,9,9,9,13],
    [13,9,9,9,10,10,10,11,12,12,12,12,12,9,9,9,13],
    [13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13,13]
];
